...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID )
		...
		$link = "<a class='comment-reply-link gray' href='" . esc_url( add_query_arg( 'replytocom', $comment->comment_ID ) ) . "#" . $respond_id . "' onclick='return addComment.moveForm(\"$add_below-$comment->comment_ID\", \"$comment->comment_ID\", \"$respond_id\", \"$post->ID\")'><span>$reply_text</span></a><div class='clear'></div>";
...
function get_post_reply_link($args = array(), $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID ) {
		...
		$link = "<a rel='nofollow' class='comment-reply-link gray' href='" . get_permalink($post->ID) . "#$respond_id' onclick='return addComment.moveForm(\"$add_below-$post->ID\", \"0\", \"$respond_id\", \"$post->ID\")'><span>$reply_text</span></a><div class='clear'></div>";
	}
...
function get_cancel_comment_reply_link($text = '') {
	...
	return apply_filters('cancel_comment_reply_link', '<a rel="nofollow" id="cancel-comment-reply-link" href="' . $link . '"' . $style . '><span>' . $text . '</span></a><div class="clear"></div>', $link, $text);
}
